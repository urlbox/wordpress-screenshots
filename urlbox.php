 ...
 * Plugin URI: http://wordpress.org/extend/plugins/urlbox-screenshots/
...
if (!class_exists('Urlbox')) {
	class Urlbox {
		...
		public function add_plugin_settings_link($links, $file){
			...
			if ($file == $this_plugin) {
				$url = admin_url('options-general.php?page=urlbox_slug');
		...
		public function render_admin_page() {
			...
			<div class='wrap'>
				...
				<?php
					...
					do_settings_sections('urlbox_slug');
		...
		public function add_fields() {
			...
			add_settings_section(
				...
				'urlbox_slug' // menu slug (see add_options_page)
			);
			add_settings_section(
				'default',
				'Default Options',
		...
		public function printHtmlSectionInfo()
    {
		...
		public function generateUrl($urlboxOptions) {
			...
			foreach ($urlboxOptions as $key => $value) {
				if (!in_array($key,array('api','secret','format', 'customcss', 'imagesize', 'div_class', 'img_class'))) {
					if(!empty($value)){
        		$_parts[] = "$key=$value";
					}
				}
